...
class BESSWorkflowController:
    ...
    def calculate_dam_profits(self, power_mw, energy_mwh):
        ...
        dam_file = f'{self.base_path}/cleaned_market_prices/dam_and_idm/2024_dam_idm_price.csv'
    ...
    def run_baseline_stage_with_dc_scaling(self, power_mw, energy_mwh, price_scale_factor=1.0, scenario_name="base"):
        ...
        # Calculate key statistics for export
        ...
        if result:
            return {
                ...
                'profits_statistics': profits_stats  # Key statistics instead of raw data
            }
        return None
    ...
    def run_single_config_dc_price_levels(self, config):
        ...
        config_results = []
        config_profit_stats = []  # Store profit statistics for each scenario
        ...
        for scenario_name, scale_factor in price_scenarios.items():
            ...
            if baseline_result:
                ...
                # Extract profit statistics
                stats = baseline_result['profits_statistics']
                stats_record = {
                    'config_id': config_id,
                    'power_mw': power_mw,
                    'energy_mwh': energy_mwh,
                ...
                config_profit_stats.append(stats_record)
        ...
        return config_results, config_profit_stats
    ...
    def run_all_configurations_dc_price_levels(self):
        ...
        for config in BESS_CONFIGS:
            config_results, config_profit_stats = self.run_single_config_dc_price_levels(config)
            if config_results:
                all_results.extend(config_results)
                all_profit_stats.extend(config_profit_stats)
        
        # Save consolidated results
        if all_results:
            output_dir = f'{self.base_path}/fig_14_appendix_9_model_outputs'
            os.makedirs(output_dir, exist_ok=True)
            
            # 1. Save allocation results
            results_df = pd.DataFrame(all_results)
            allocation_file = os.path.join(output_dir, '2024_DC_price_level_allocations.csv')
            results_df.to_csv(allocation_file, index=False)
            
            # 2. Save profit statistics (key stats instead of raw daily data)
            if all_profit_stats:
                stats_df = pd.DataFrame(all_profit_stats)
                stats_file = os.path.join(output_dir, 'fig_14_appendix_9_data.csv')
                stats_df.to_csv(stats_file, index=False)
                
                print(f"\nProfit statistics data saved to: {stats_file}")
                print(f"Statistics records: {len(stats_df)}")
            
            print(f"\nDC Price Level allocation results saved to: {allocation_file}")
            print(f"Total allocation results: {len(all_results)}")
            print(f"Configurations processed: {len(BESS_CONFIGS)}")
            print(f"Price scenarios per config: {len(set(r['scenario'] for r in all_results))}")
            
            print("\nExported statistics include:")
            print("- Mean daily profits for DAM, IDM, DC markets")
            print("- Standard deviations for each market")
            print("- Correlation coefficients between all market pairs")
            print("- Number of trading days used in calculations")
        
        return all_results, all_profit_stats
...
def main():
    ...
    allocation_results, profit_statistics = controller.run_all_configurations_dc_price_levels()
    return allocation_results, profit_statistics

if __name__ == "__main__":
    allocation_results, profit_statistics = main()