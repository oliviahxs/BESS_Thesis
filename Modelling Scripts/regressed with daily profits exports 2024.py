...
class BESSWorkflowController:
    ...
    def calculate_dam_profits(self, power_mw, energy_mwh):
        ...
        csv_file = f'{self.base_path}/Cleaned market prices/DAM & IDM/2024 DAM IDM Price.csv'
    ...
    def predict_idm_prices(self):
        ...
        for year in [2021, 2022, 2023]:
            ...
            dam_idm_file = f'{self.base_path}/Cleaned market prices/DAM & IDM/{year} DAM IDM Price.csv'
            ...
            output_file = f'{self.base_path}/FINAL MODEL/predicted_idm_prices_{year}_from_{year}dam.csv'
    ...
    def calculate_idm_profits(self, year, power_mw, energy_mwh, dam_weight):
        ...
        predicted_file = f'{self.base_path}/FINAL MODEL/predicted_idm_prices_{year}_from_{year}dam.csv'
    ...
    def run_baseline_stage(self, power_mw, energy_mwh):
    ...
    def run_updated_stage(self, power_mw, energy_mwh, dam_weight):
        ...
        # Calculate DC profits with remaining capacity (same for all years)
        dc_df = self.calculate_dc_profits(dam_weight, power_mw)
        # Add month-day column for matching
        dc_df['month_day'] = pd.to_datetime(dc_df['Date']).dt.strftime('%m-%d')
        
        for year in [2021, 2022, 2023]:
            # Calculate IDM profits with remaining capacity
            idm_df = self.calculate_idm_profits(year, power_mw, energy_mwh, dam_weight)
            
            # Store daily profits data for export
            for idx, idm_row in idm_df.iterrows():
                # Find matching DC profit by month-day
                dc_profit = 0
                idm_date = pd.to_datetime(idm_row['Date'])
                idm_month_day = idm_date.strftime('%m-%d')
                matching_dc = dc_df[dc_df['month_day'] == idm_month_day]
                if not matching_dc.empty:
                    dc_profit = matching_dc.iloc[0]['Daily DC returns']
                
                # Calculate day of year
                day_of_year = idm_date.dayofyear
                
                daily_profits_data.append({
                    'power': power_mw,
                    'energy': energy_mwh, 
                    'duration': energy_mwh / power_mw,
                    'day_of_year': day_of_year,
                    'year': year,
                    'daily_IDM_profit': idm_row[f'IDM_Daily_Profit_{year}'],
                    'daily_DC_profit': dc_profit
                })
            
            # Merge by date matching for optimization
        ...
        return results, daily_profits_data

    def run_single_config(self, config):
        """Run complete workflow for a single BESS configuration"""
        ...
        baseline_result = self.run_baseline_stage(power_mw, energy_mwh)
        
        if not baseline_result:
            print(f"  Baseline optimization failed for Config {config_id}")
            return None, None
        ...
        updated_results, daily_profits_data = self.run_updated_stage(power_mw, energy_mwh, dam_weight)
        ...
        print(f"  Config {config_id} completed successfully")
        return result, daily_profits_data

    def run_all_configurations(self):
        """Run workflow for selected BESS configurations (1, 5, 8)"""
        ...
        # Filter to only run configs 1, 5, 8
        selected_configs = [config for config in BESS_CONFIGS if config['config_id'] in [1, 5, 8]]
        
        for config in selected_configs:
            result, daily_profits_data = self.run_single_config(config)
            if result:
                all_results.append(result)
                all_daily_profits.extend(daily_profits_data)
        ...
        if all_results:
            ...
            output_file = os.path.join(output_dir, 'consolidated_bess_results_2024_base.csv')
            results_df.to_csv(output_file, index=False)
            
            # Save daily profits data
            if all_daily_profits:
                daily_profits_df = pd.DataFrame(all_daily_profits)
                daily_profits_file = os.path.join(output_dir, 'daily_profits_regressed.csv')
                daily_profits_df.to_csv(daily_profits_file, index=False)
                
                print(f"\nDaily profits data saved to: {daily_profits_file}")
