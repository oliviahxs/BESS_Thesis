...
class BESSWorkflowController:
    ...
    def calculate_dc_profits_with_addition(self, power_mw, price_addition_percent=0.0):
        """Calculate DC profits using 2024 prices with additive adjustment based on percentage of mean price"""
        ...
        def calculate_dc_returns(csv_file, service_type):
            ...
            # Calculate mean price for additive adjustment
            mean_price = df['Clearing Price'].mean()
            price_addition = mean_price * (price_addition_percent / 100.0)
            
            # Apply additive price adjustment
            df['Adjusted_Price'] = df['Clearing Price'] + price_addition
            df['EFA_Return'] = df['Adjusted_Price'] * 4 * dc_bid_size
    ...
    def run_baseline_stage_with_dc_addition(self, power_mw, energy_mwh, price_addition_percent=0.0, scenario_name="base"):
        """Run baseline stage (3-asset optimization using 2024 data) with DC price addition"""
        ...
        # Calculate DC profits with price addition
        dc_df = self.calculate_dc_profits_with_addition(power_mw, price_addition_percent)
        ...
        dam_std = np.std(dam_returns, ddof=1)
        idm_std = np.std(idm_returns, ddof=1)
        dc_std = np.std(dc_returns, ddof=1)
        ...
        return {
            ...
            'price_addition_percent': price_addition_percent,
            'dam_mean': round(dam_mean, 4),
            'idm_mean': round(idm_mean, 4),
            'dc_mean': round(dc_mean, 4),
            'dam_std': round(dam_std, 4),
            'idm_std': round(idm_std, 4),
            'dc_std': round(dc_std, 4),
            'corr_dam_idm': round(corr_matrix[0,1], 4),
            'corr_dam_dc': round(corr_matrix[0,2], 4),
            'corr_idm_dc': round(corr_matrix[1,2], 4),
            'num_days': len(merged)
    ...
    def run_single_config_dc_price_levels(self, config):
        """Run baseline optimization across different DC price levels using additive adjustments"""
        config_id = config['config_id']
        power_mw = config['power_mw']
        energy_mwh = config['energy_mwh']
        ...
        # Define price addition scenarios (percentage of mean price)
        price_scenarios = {
            'base_case': 0.0,
            'down_80%': -80.0,
            'down_75%': -75.0,
        ...
        for scenario_name, addition_percent in price_scenarios.items():
            print(f"  Running {scenario_name} scenario (addition: {addition_percent:+.1f}%)")
            
            baseline_result = self.run_baseline_stage_with_dc_addition(
                power_mw, energy_mwh, addition_percent, scenario_name
            )
            ...
            if baseline_result:
                result = {
                    ...
                    'energy_mwh': energy_mwh,
                    'scenario': scenario_name,
                    'dc_price_addition_percent': addition_percent,
                    'dam_mean': baseline_result['dam_mean'],
                    'idm_mean': baseline_result['idm_mean'],
                    'dc_mean': baseline_result['dc_mean'],
                    'dam_std': baseline_result['dam_std'],
                    'idm_std': baseline_result['idm_std'],
                    'dc_std': baseline_result['dc_std'],
                    'corr_dam_idm': baseline_result['corr_dam_idm'],
                    'corr_dam_dc': baseline_result['corr_dam_dc'],
                    'corr_idm_dc': baseline_result['corr_idm_dc'],
                    'num_days': baseline_result['num_days']
                }
                config_results.append(result)
                print(f"    DAM μ: {result['dam_mean']:.2f}, IDM μ: {result['idm_mean']:.2f}, DC μ: {result['dc_mean']:.2f}")
            else:
    ...
    def run_all_configurations_dc_price_levels(self):
        """Run baseline optimization for all configurations across DC price levels using additive adjustments"""
        print("Starting BESS DC Price Level Analysis - Additive Price Adjustments")
        print("=" * 70)
        ...
        if all_results:
            ...
            # Export market statistics for each DC price scenario
            market_stats_file = os.path.join(output_dir, 'D*_price_addition_profit_stats.csv')
            results_df.to_csv(market_stats_file, index=False)
            
            print(f"\nDC Price Addition Market Statistics saved:")
            print(f"  Market statistics: {market_stats_file}")
            print(f"Total scenario results: {len(all_results)}")
            print(f"Configurations processed: {len(BESS_CONFIGS)}")
            print(f"Price scenarios per config: 23 (base + ±5% to ±80%)")
            print("\nExported data includes for each configuration and DC price scenario:")
            print("- Mean profits for DAM, IDM, DC markets")
            print("- Standard deviations (risk) for each market")
            print("- Correlation coefficients between markets")
            print("- Number of trading days used")
