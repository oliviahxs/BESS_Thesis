...
class BESSWorkflowController:
    ...
    # may delete if data already converted into UK time!!!
    def parse_datetime(self, date_string):
        """Parse datetime from CSV - data is already in UK time"""
        return datetime.strptime(date_string, '%d/%m/%Y %H:%M')

    # Daily arbitrage optimiser using CPLEX
    ...
    # Calculate DAM profits from the arbitrage optimiser
    ...
    def predict_idm_prices(self):
        for year in [2021, 2022, 2023]:
            ...
            output_file = f'{self.base_path}/Modelling Scripts/predicted_idm_prices_{year}_from_{year}dam.csv'
    ...
    def calculate_idm_profits(self, year, power_mw, energy_mwh, dam_weight):
        predicted_file = f'{self.base_path}/Modelling Scripts/predicted_idm_prices_{year}_from_{year}dam.csv'
    ...
    # Merge IDM and DC profits by matching on the same date of year
    ...
    # Run workflow for all configurations
    def run_all_configurations(self):
        ...
        if all_results:
            output_dir = f'{self.base_path}/Modelling Scripts/Results'
