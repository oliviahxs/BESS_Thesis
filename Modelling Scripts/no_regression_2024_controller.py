...
class BESSWorkflowController:
    ...
    def calculate_idm_profits(self, year, power_mw, energy_mwh, dam_weight):
        """Calculate IDM profits using actual IDM prices and remaining capacity"""
        idm_file = f'{self.base_path}/Cleaned market prices/DAM & IDM/{year} DAM IDM Price.csv'
    ...
    def run_all_configurations(self):
        ...
        if all_results:
            ...
            output_file = os.path.join(output_dir, 'no_regression_2024_results.csv')
            results_df.to_csv(output_file, index=False)
            
            # Save daily profits data
            if all_daily_profits:
                daily_profits_df = pd.DataFrame(all_daily_profits)
                daily_profits_file = os.path.join(output_dir, 'daily_profits_updated_stage.csv')
