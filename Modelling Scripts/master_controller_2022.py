"""
Master Workflow Controller for BESS Arbitrage Optimization (2022 Base Case)
Integrates all stages of the two-stage portfolio optimization process
Base case: 2022, Scenario years: 2024, 2023, 2021
"""
...
class BESSWorkflowController2022:
    ...
    def calculate_dam_profits(self, power_mw, energy_mwh):
        """Calculate DAM daily profits for 2022"""
        csv_file = f'{self.base_path}/Cleaned market prices/DAM & IDM/2022 DAM IDM Price.csv'
    ...
    def predict_idm_prices(self):
        """Generate IDM price predictions for 2024, 2023, 2021 using their actual DAM prices"""
        ...
        for year in [2024, 2023, 2021]:
            ...
            output_file = f'{self.base_path}/Streamlined Workflow/predicted_idm_prices_{year}_from_{year}dam.csv'
    ...
    def calculate_idm_profits(self, year, power_mw, energy_mwh, dam_weight):
        ...
        predicted_file = f'{self.base_path}/Streamlined Workflow/predicted_idm_prices_{year}_from_{year}dam.csv'
    ...
    def calculate_dc_profits(self, dam_weight, power_mw):
        """Calculate DC profits using 2022 prices and remaining capacity"""
        dc_power_capacity = power_mw * (1 - dam_weight)
        bid_size = 0.8 * dc_power_capacity
        
        dc_path = f'{self.base_path}/Cleaned market prices/DC'
        dch_file = os.path.join(dc_path, 'DCH/2022 DCH Price.csv')
        dcl_file = os.path.join(dc_path, 'DCL/2022 DCL Price.csv')
    ...
    def run_baseline_stage(self, power_mw, energy_mwh):
        """Run baseline stage (3-asset optimization using 2022 data)"""
        # Calculate DAM profits
        dam_df = self.calculate_dam_profits(power_mw, energy_mwh)
        
        # Calculate baseline IDM profits (using actual 2022 IDM prices)
        idm_file = f'{self.base_path}/Cleaned market prices/DAM & IDM/2022 DAM IDM Price.csv'
    ...
    def run_updated_stage(self, power_mw, energy_mwh, dam_weight):
        ...
        # Generate IDM predictions for 2024, 2023, 2021
    ...
    def run_single_config(self, config):
        ...
        updated_results = self.run_updated_stage(power_mw, energy_mwh, dam_weight)
    ...
    def run_all_configurations(self):
        """Run workflow for all 17 BESS configurations"""
        print("Starting BESS Portfolio Optimization Workflow (2022 Base Case)")
        print("=" * 60)
        ...
        if all_results:
            output_dir = f'{self.base_path}/Streamlined Workflow/Results'
            os.makedirs(output_dir, exist_ok=True)
            
            results_df = pd.DataFrame(all_results)
            output_file = os.path.join(output_dir, 'consolidated_bess_results_2022_base.csv')
...
def main():
    ...
    controller = BESSWorkflowController2022(base_path)
